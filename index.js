...
const createHtmlGood = (data) =>{
    return `
    <li class="goods-listItem wow slideInLeft">
        <div>
            ...
            <div class="good-listItem-info">
                ...
                <p class="good-listItem__ingredients">${data.ingredients.join(', ')}</p>
            </div>
        </div>
...
const validation = () => {
    ...
    controls.forEach(ctrl=>{
        ...
        if(ctrl.name === 'phone' && ctrl.value.length!==17 && ctrl.value.length) {
...
form.onsubmit = (e) =>{
    ...
    }).then(() => showHidePopup()).catch(error=>console.log(error))
}
